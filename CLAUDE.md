...
- `uv run student-bot-notify [--to '@user'] [--severity ok|warn|critical]` — send a report to every channel configured under `notify:` in `config.yaml` (Mattermost as the bot; ntfy push, topic from `NTFY_TOPIC` in `.env`). Severity drives ntfy priority. `-n` resolves targets without sending.
