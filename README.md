...
| **Add a user** | `uv run student-bot-mkuser alice` — prompts for a password, writes a scrypt-hashed record. Add `--admin` for the admin views on `/stats` (the per-user table, Inspect, diagnostics). |
